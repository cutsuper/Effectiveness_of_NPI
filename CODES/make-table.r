...
source('codes/format-data.r')

make_table <- function(filename){
  load(paste0("results/", filename,'-stanfit.Rdata'))
  out <- rstan::extract(fit)
  inf_mean_delta <- vector("list", length = 42)
  inf_lower_delta <- vector("list", length = 42)
  inf_upper_delta <- vector("list", length = 42)
  
  inf_mean_model <- vector("list", length = 42)
  inf_lower_model <- vector("list", length = 42)
  inf_upper_model <- vector("list", length = 42)
  
  inf_mean_null <- vector("list", length = 42)
  inf_lower_null <- vector("list", length = 42)
  inf_upper_null <- vector("list", length = 42)
  
  mean_delta <- vector("list", length = 42)
  lower_delta <- vector("list", length = 42)
  upper_delta <- vector("list", length = 42)
  
  mean_null <- vector("list", length = 42)
  lower_null <- vector("list", length = 42)
  upper_null <- vector("list", length = 42)
  
  mean_model <- vector("list", length = 42)
  lower_model <- vector("list", length = 42)
  upper_model <- vector("list", length = 42)
  ...
  write.csv(df_delta[,c('countries', 'text')], file = paste0('results/deaths-averted-',filename, '.csv'))
  ...
  write.csv(df_null[,c('countries', 'text')], file = paste0('results/deaths-null-',filename, '.csv'))
  ...
  write.csv(df_model[,c('countries', 'text')], file = paste0('results/deaths-model-',filename, '.csv'))
  ...
  df_model['text'] <- paste0(df_model[,1]," [",df_model[,2]," - ",df_model[,3],"]")
  df_model <- cbind(df_model, countries)
  write.csv(df_model[,c('countries', 'text')], file = paste0('results/cases-model-',filename, '.csv'))
  
  #df_pop = readRDS('data/popt-ifr.rds')
  #df_pop = df_pop[1:11, c('country', 'popt')]
  #df_pop <- df_pop[match(countries[1:11], df_pop$country),]
  pop = unique(read.csv('data/death_data_upto613.csv')[,10])
  X = 1:51
  ifr.by.country = cbind(X,read.csv('data/ifr_by_state.csv')[,c(2,3)],pop)
  colnames(ifr.by.country) = c("X", "country", "ifr","popt")
  df_pop = ifr.by.country[,c(2,4)]
  outlier = c(2,12,13,27,35,42,46,49,51)
  df_pop = df_pop[-outlier,]
  ...
  df_model['text'] <- paste0(df_model[,1]," [",df_model[,2]," - ",df_model[,3],"]")
  df_model <- cbind(df_model, countries)
  write.csv(df_model[,c('countries', 'text')], file = paste0('results/per-cases-model-',filename, '.csv'))
}
