...
#    index1 = which(cumsum(region$Deaths)>=15)[1] # also 5
...
#    firstIntervention = 1*((school+ selfIsolation+ publicEvents+ lockdown + socialDistancing) >= 1)
    ...
    gather =region_intervention[,1]
    other =region_intervention[,2]
    rest = region_intervention[,3]
    school = region_intervention[,4]
    home = region_intervention[,5]
    firstIntervention = 1*((gather+other+rest+school+home) >= 1)
    ## append data
    ...
    df_features = data.frame('gather' = gather, 'other' = other, 'rest' = rest,
                             'firstIntervention' = firstIntervention, 'school' = school, 'home' = home)
