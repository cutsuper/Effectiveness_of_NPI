...
source("codes/geom-stepribbon.r")
#---------------------------------------------------------------------------
make_three_panel_plot <- function(filename){
  ...
  for(i in 1:length(countries$Regions)){
    print(i)
    N <- length(dates[[i]])
 #   country <- countries[[i]]
    country = countries$Regions[i]
...
make_plots <- function(data_country, covariates_country_long, 
  ...
  p3 <- ggplot(data_country) +
    ...
    ylab(expression(R[t])) +
    scale_fill_manual(name = "", labels = c("50%", "95%"),
                      values = c(alpha("lightpink", 0.75), alpha("lightpink", 0.5))) + 
  ...
  save_plot(filename = paste0("figures/", country, "-three-pannel-", filename, ".png"),
            p, base_width = 14)
}
